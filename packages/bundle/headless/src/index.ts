...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`CurupiraCode: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`CurupiraCode: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
