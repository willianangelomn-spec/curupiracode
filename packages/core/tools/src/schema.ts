...
/**
 * Wrap a lone string in a one-item array for string-array parameters.
 *
 * Some models emit `{"queries": "term"}` where the schema declares
 * `{"queries": ["term"]}` — a scalar in an array slot. Observed repeatedly
 * with `web_search` (a model looped eight times on the same call, its own
 * reasoning acknowledging the array requirement while still emitting a
 * string, then abandoned the tool). The single-item intent is unambiguous,
 * so the call proceeds instead of failing.
 *
 * Deliberately narrow: only a top-level property whose schema is exactly
 * `array` of `string`, and only when the supplied value is a string. Every
 * other shape mismatch still reaches the validator and fails, so genuine
 * model errors stay visible rather than being silently reinterpreted.
 * @param schema - the compiled object-rooted parameter schema.
 * @param args - candidate arguments, however malformed.
 * @returns Arguments with eligible scalars wrapped; the input when nothing applies.
 */
function coerceLoneStringsIntoArrays(schema: ObjectJsonSchema, args: unknown): unknown {
  if (typeof args !== 'object' || args === null || Array.isArray(args)) return args
  const properties = schema.properties
  if (properties === undefined) return args
  let coerced: Record<string, unknown> | undefined
  for (const [key, node] of Object.entries(properties)) {
    if (node.type !== 'array' || node.items?.type !== 'string') continue
    const value = (args as Record<string, unknown>)[key]
    if (typeof value !== 'string') continue
    coerced ??= { ...args as Record<string, unknown> }
    coerced[key] = [value]
  }
  return coerced ?? args
}
...
export function defineTool<const S extends ParameterSchemaSpec, const O extends ValueSchemaSpec>(
  ...
  const tool: ToolDefinition = {
    ...
    async execute(args: unknown, exec: ToolRunContext): Promise<JsonValue> {
      const candidate = coerceLoneStringsIntoArrays(parameters, args)
      const violations = validate(candidate)
      if (violations.length > 0) throw new ToolArgsError(violations)
      return userExecute(candidate as InferArgs<S>, exec) as Promise<JsonValue>
    },
  }
  ...
  // than the hard `ToolArgsError` the execute path raises. These paths coerce
  // exactly like `execute` because the log stores the model's raw arguments: a
  // call the harness accepted must not then replay as a generic card.
  if (userPresentCall) {
    tool.presentCall = (args: unknown): ToolCallView | undefined => {
      const candidate = coerceLoneStringsIntoArrays(parameters, args)
      if (validate(candidate).length > 0) return undefined
      return userPresentCall(candidate as InferArgs<S>)
    }
  }
  if (userPresentResult) {
    tool.presentResult = (args: unknown, result: ToolResult): ToolResultView | undefined => {
      const candidate = coerceLoneStringsIntoArrays(parameters, args)
      if (validate(candidate).length > 0) return undefined
      return userPresentResult(candidate as InferArgs<S>, result)
    }
  }
  if (userIsConcurrencySafe) {
    tool.isConcurrencySafe = (args: unknown): boolean => {
      const candidate = coerceLoneStringsIntoArrays(parameters, args)
      if (validate(candidate).length > 0) return false
      return userIsConcurrencySafe(candidate as InferArgs<S>)
    }
  }
  return tool
}
