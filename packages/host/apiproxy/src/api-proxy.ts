    ...
    authorization: {
      async list(request) {
        ...
        if (auth === undefined) {
          return err(request, {
            code: 'internal',
            message: 'authorization service is not mounted',
            details: {},
          })
        }
        const credentials = ctx.get('credentials')
        const flows = auth.list()
        const views = []
      ...
      async begin(request) {
        ...
        const flow = auth.describe(key as CredentialKey)
        if (flow === undefined) {
          return err(request, {
            code: 'internal',
        ...
        let firstNoticeResolved = false
        let resolveFirstNotice: (notice: AuthorizationNotice) => void = () => undefined
        const firstNotice = new Promise<AuthorizationNotice>((resolve) => { resolveFirstNotice = resolve })
        const interaction = {
          notify: (notice: AuthorizationNotice) => {
            if (firstNoticeResolved) return
            firstNoticeResolved = true
            resolveFirstNotice(notice)
          },
          prompt: () => Promise.reject(
            new Error('authorization prompts are not surfaced in this build yet')),
        }
        const attempt = auth.begin({ key: key as CredentialKey, method, interaction })
      ...
      cancel(request) {
        ...
        auth.cancel(key as CredentialKey)
        return Promise.resolve(ok(request, {}))
      },
    },
