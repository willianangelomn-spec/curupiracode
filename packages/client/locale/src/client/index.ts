 ...
 */
const DOCUMENT_LANGUAGE: Record<LocaleId, string> = { zh: 'zh-CN', en: 'en', 'pt-BR': 'pt-BR' }
...
/**
 * The first shipped locale the browser asks for: a full tag matching a shipped
 * id wins (`pt-BR` -> pt-BR), otherwise the primary subtag lands every
 * regional variant on its language (`zh-Hans-CN` -> zh, `en-GB` -> en, bare
 * `pt` -> pt-BR). `window` is the browser test, not `navigator`: Node exposes
...
function detectBrowserLocale(): LocaleId | undefined {
  ...
  for (const tag of [...(navigator.languages ?? []), navigator.language]) {
    ...
    // A full tag may name a shipped locale directly ('pt-br' -> pt-BR).
    const exact = LOCALES.find(locale => locale.id.toLowerCase() === lowered)
    if (exact) return exact.id
    // Otherwise regional variants land on their language's shipped locale
    // ('zh-Hans-CN' -> zh, 'en-GB' -> en, bare 'pt' -> pt-BR).
    const primary = lowered.split('-')[0]
    const match = LOCALES.find(locale => locale.id.toLowerCase().split('-')[0] === primary)
