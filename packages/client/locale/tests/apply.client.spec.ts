...
describe('locale apply', () => {
  ...
  it('selects, persists, and re-adopts pt-BR', async () => {
    const b = await bench()
    declareItems(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const locale = b.ctx.get('locale') as LocaleRuntime
    ...
    expect(locale.bind(SETTINGS_NS)('language.title')).toBe('Idioma')
    await vi.waitFor(() => { expect(b.mutate).toHaveBeenCalled() })
    // An externally stored preference re-adopts after a document update.
    b.setHostPreference('pt-BR')
    b.ctx.remote.$dispatch('settings/document-updated', [LOCALE_SETTINGS_NAMESPACE, 0])
    await vi.waitFor(() => { expect(locale.getLocale().active).toBe('pt-BR') })
  })
