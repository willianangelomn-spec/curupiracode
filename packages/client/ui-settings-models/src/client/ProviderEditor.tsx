...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  // Gemini signs in through the authorization seam rather than an API key, so
  // its card offers a login button instead of a key input.
  const gemini = namespace.ns === 'llm-gemini'
  const [authEntry, setAuthEntry] = useState<{
    key: string; label: string; methods: { id: string; label: string }[]; configured: boolean
  } | undefined>(undefined)
  const [authBusy, setAuthBusy] = useState(false)
  const [authNotice, setAuthNotice] = useState<string | undefined>(undefined)
  // True between "sign-in started" and either the flow reporting configured or
  // the wait window elapsing; drives the polling that flips the card to its
  // connected state the moment Google's round-trip lands.
  const [awaitingSignIn, setAwaitingSignIn] = useState(false)
  useEffect(() => {
    if (!gemini) return
    const auth = api.authorization
    if (auth === undefined) {
      setAuthNotice('Login indisponível: face de autorização não exposta.')
      return
    }
    let stale = false
    void auth.list({}).then((response) => {
      if (stale) return
      if (!response.result.ok) { setAuthNotice(response.result.error.message); return }
      const flow = response.result.value.flows.find(
        candidate => candidate.key === `${namespace.ns}/${props.provider}`,
      )
      if (flow !== undefined) setAuthEntry(flow)
      else setAuthNotice('Nenhum fluxo de login para este provider.')
    }, (error) => { if (!stale) setAuthNotice(messageOf(error)) })
    return () => { stale = true }
  }, [gemini, api.authorization, namespace.ns, props.provider])
  useEffect(() => {
    ...
    const giveUp = setTimeout(() => {
      if (!stopped) setAwaitingSignIn(false)
    }, 120_000)
    return () => { stopped = true; clearInterval(poll); clearTimeout(giveUp) }
  }, [awaitingSignIn, api.authorization, namespace.ns, props.provider])
  const signIn = async (): Promise<void> => {
    if (authEntry === undefined || api.authorization === undefined) {
      setAuthNotice('Fluxo de login não carregado — recarregue a página.')
      return
    }
    const method = authEntry.methods[0]?.id ?? 'oauth'
    setAuthBusy(true)
    setAuthNotice(undefined)
    try {
      const response = await api.authorization.begin({ key: authEntry.key, method })
      if (!response.result.ok) { setAuthNotice(response.result.error.message); return }
      const view = response.result.value
      if (view.url !== undefined && typeof window !== 'undefined') {
        window.open(view.url, '_blank', 'noopener')
        setAuthNotice('Autorize no Google; este card mostra "Conectado" quando terminar.')
        setAwaitingSignIn(true)
      } else {
        setAuthNotice(view.message ?? 'Login iniciado.')
      }
    } catch (error) {
      setAuthNotice(messageOf(error))
    } finally {
      setAuthBusy(false)
    }
  }
    ...
    <div className={props.credentialOnly === true ? styles['addBlock'] : styles['editor']}>
      ...
      {layout === 'unknown'
        ? gemini
          ? (
            <div className={styles['field']}>
              <span className={styles['fieldLabel']}>{props.displayName}</span>
              {authEntry?.configured === true
                ? <p className={styles['advancedHint']}>Conectado à conta Google</p>
                : (
                  <button
                    type="button"
                    disabled={authBusy}
                    onClick={() => { void signIn() }}
                    style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid #8888', background: '#ffffff', color: '#333', cursor: 'pointer' }}
                  >
                    {authBusy ? 'Abrindo…' : 'Entrar com Google'}
                  </button>
                )}
              {authNotice !== undefined ? <p className={styles['error']}>{authNotice}</p> : null}
            </div>
          )
