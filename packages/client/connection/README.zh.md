...
`trustedExtensionIds` 为前台 Chromium 扩展页面增加一条更窄、仅限回环的例外。只有精确的 `chrome-extension://<id>` 来源，且 HTTP `Host` 为回环地址时，才可进行预检并 POST RPC 请求；其他方法、WebSocket upgrade、普通网页来源、非回环权威、格式错误或未声明的扩展 id 仍会被拒绝。随 CurupiraCode 发布的 id 是默认值。会打开桌面、暴露或修改配置与凭据、或创作 preset 的 RPC 仍需通过内层同源回环检查，因此扩展无法调用。这项稳定来源授权只是开发桥梁，不代表认证；在远程或多用户部署前，仍必须实现可见的 token 配对。
