...
内置 `gemini` 路由上支持图片的模型使用相同的确定性请求版本和从旧到新的总量限制。首选 Antigravity 传输会把这些确切字节写入请求专属的私有临时目录，在扁平化提示中按原位置保留 `@path` 引用，仅通过 `--add-dir` 向 `agy` 授权该目录，并在子进程结束后删除目录。没有已准备请求版本的图片会明确失败，绝不会被静默丢弃。如果 `agy` 不可用且 Google OAuth 回退已获授权，Code Assist 会以 Gemini `inlineData` 部件接收保留版本。目录元数据未包含 `image` 输入模态的模型会在提供方执行前拒绝新提交的图片；持久历史图片仍遵循共享请求投影规则。
...
包测试会生成 16-bit RGB 和 RGBA PNG fixture，验证 8-bit 转换与干净 8-bit 字节直通、字节压力下保留透明通道、区分高频和普通照片与低色数图形、首个候选合规后停止编码、正方形和宽屏 640,000 像素投影、请求字节不超过 1MiB、相同变体与上传 singleflight 且不会共享取消、变换并发上限、缓存与上传身份、跳过已保守 offload 的历史附件读取、批量只准备一次、Files 响应不一致、进入刷新余量时不查询远端并更新 ID、单个 ID、多个 ID 和模糊失效响应只恢复一次、文件解析失败后回退到有界全内联请求、删除配额文件前完成分页、规范化提供方诊断、纯文本投影，以及普通请求与压缩共享请求字节。Gemini 专项测试验证保守 offload、确切请求图片读取、Antigravity 私有暂存和清理、拒绝丢弃未暂存图片，以及 Code Assist `inlineData` 序列化。带凭据的 Antigravity 冒烟测试通过 `@path` 引用读取真实存储图片。无需密钥的组装快照覆盖真实工具 schema 和图片请求路径。带凭据的 DeepSeek 测试只使用内置 `deepseek-official` 路由及其已配置端点，不使用自定义提供方条目。

## Consequences

持久规范化附件最多占用独立的本地安全上限，请求缓存、瞬时 Gemini 暂存和远端 Files 还会占用额外派生存储。确定性身份和 singleflight 使这些成本可以被共享同一 DSH home 的轮次和会话复用。Gemini 暂存只在一次 `agy` 调用期间复制有界请求版本，随后删除。同时执行两个变换会降低批次延迟，但峰值 RSS 高于串行执行；内存更紧张的部署可以把上限设为 1。编码器或变换策略版本变化会为未来内容产生新身份，不会改写已有历史。DeepSeek 图片请求优先复用 Files；有界的陈旧 ID 恢复会处理远端状态不一致，文件解析失败则使用较小的内联预算。缺失或损坏的持久附件仍需要单独的隔离设计。
